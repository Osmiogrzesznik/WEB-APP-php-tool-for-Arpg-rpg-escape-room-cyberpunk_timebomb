...
try {
  ...
  //defaults
  timebomb_password = "0000";
  timebomb_time_set = Date.now() + 30000;
  ...
  devLocate = {
    ...
    getLocationSuffix(additionalData) {
      let suffix = this.isOKtoSend() ? 
        encodeData({latitude: this.position.latitude,
        longitude: this.position.longitude}) :
        "";
      additionalDataURL = additionalData ? ("&" + encodeData(additionalData)): "";

      return suffix + additionalDataURL;
    },
    startLocating() {
    ...
    locationSuccess(position) {
      ...
      this.sendLastLocationToServer(locsuffix);
    },
    sendLastLocationToServer(locsuffix) {
  ...
  //this should belong to separate send/receive module
  ...
  function startBombWithData(data) {
    ...
    alert("this device is " + data.timebomb_status);

    timebomb_time_set = new Date(data.timebomb_time_set);
    //alert(timebomb_time_set.getTime());
    ...
    cx.onFinished = function (c) {
      locsuf = devLocate.getLocationSuffix({name:"statuschange",value:"detonated"})
      devLocate.sendLastLocationToServer(locsuf);
      detonate();
    };
    ...
    kb.hide();
    //cx.setLimit(0, 0, 6);
    cx.setTimestampEnd(timebomb_time_set); // this sets the countdown timer to 40 seconds 
    cx.start();
  ...
  function updateStateFromServerResponse(j) {
    cx.setTimestampEnd(Date.parse(j.timebomb_time_set));
    if (j.timebomb_status == "detonated") {
      cx.stop();
      cx.showTime(0, 0, 0);
      detonate();
    }
    if (j.timebomb_status == "disarmed") {
      cx.stop();
    }
